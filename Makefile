...
ifeq ($(AVX_OT), 0)
VM += ECDSA/P256Element.o RSIG/CurveElement.o
OT += ECDSA/P256Element.o RSIG/CurveElement.o
MINI_OT += ECDSA/P256Element.o RSIG/CurveElement.o
else
...
all: arithmetic binary gen_input online offline externalIO bmr ecdsa rsig
...
rsig: $(patsubst RSIG/%.cpp,%.x,$(wildcard RSIG/*-rsig-party.cpp)) Fake-RSIG.x
rsig-static: static-dir $(patsubst RSIG/%.cpp,static/%.x,$(wildcard RSIG/*-rsig-party.cpp))
...
static/%.x: RSIG/%.o RSIG/CurveElement.o $(VMOBJS) $(OT) $(LIBSIMPLEOT)
	$(CXX) $(CFLAGS) -o $@ $^ -Wl,-Map=$<.map -Wl,-Bstatic -static-libgcc -static-libstdc++ $(BOOST) $(LDLIBS) -Wl,-Bdynamic -ldl
...
Fake-RSIG.x: RSIG/Fake-RSIG.cpp RSIG/CurveElement.o $(COMMON) Processor/PrepBase.o
	$(CXX) -o $@ $^ $(CFLAGS) $(LDLIBS)
...
fake-spdz-rsig-party.x: $(OT) $(LIBSIMPLEOT)
